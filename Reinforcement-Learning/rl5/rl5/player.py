...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        # It is recommended you use the np.random module
        scheduleLinear = ScheduleLinear(anneal_timesteps, epsilon_final, epsilon_initial)
        epsilon = scheduleLinear.value(current_total_steps) 
        if np.random.random() < epsilon:
            action = np.random.choice(all_actions)
        else:
            action = np.nanargmax(Q[state])
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        Q = np.random.random((ns, na))    # interval [0,1)
        # ADD YOUR CODE SNIPPET BETWEEN EX. 2.1
        ...
        while episode <= self.episode_max and diff > threshold:
            ...
            diff = np.abs(np.nanmean(Q - Q_old)) # element wise difference -> mean of diff array -> abs value
        
            R_recent_rewards.append(R_total)
            if len(R_recent_rewards) >= N_episodes:
                # All rewards in the last N episodes are identical
                if all(reward == R_recent_rewards[0] for reward in R_recent_rewards):
                    break  # converged

            # Clear after 10 episodes to track the next group of episodes
            if episode % N_episodes == 0:
                R_recent_rewards.clear()
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        return self.initial_p + (self.final_p - self.initial_p) * (t / self.schedule_timesteps) # initial + delta * t/T
        # ADD YOUR CODE SNIPPET BETWEEN EX 4.2
